...
def pseudo_golden_all_gather(
    ...
    first_group_tensors = []
    for id in replica_groups[0]:
        first_group_tensors.append(shards[id])
    output_tensor = torch.cat(first_group_tensors, dim=all_gather_dim)
...
@pytest.mark.parametrize(
    ...
    list(
        test_util.sweep_all_sharding_configs(
            input_rank=4, num_devices=8, min_mesh_dim=2, max_mesh_dim=2
        )
    ),
)
...
def test_all_gather(
    ...
    def all_gather(in0: Operand, builder: TTIRBuilder):
        ...
        golden_output = pseudo_golden_all_gather(
            input, mesh_shape, shard_dims, all_gather_dim, cluster_axis, replica_groups
        )
